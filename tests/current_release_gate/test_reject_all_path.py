"""One deterministic bar-to-refusal path for the current source identity."""
...
class _FailClosedGateway:
    """A local failure double, never a provider client or network transport."""
    ...
    async def complete(self, *, workload, **_kwargs):
        ...
        raise RuntimeError("synthetic local gateway outage")


class _NoVenueAccess:
    """Any venue touch after a rejected decision is an integration failure."""

    name = "must-not-be-called"

    def __getattr__(self, name: str):  # pragma: no cover - asserted by this path
        raise AssertionError(f"rejected decision accessed venue method {name}")


def _closed_bars() -> tuple:
    """Twenty-five complete UTC hours that deterministically emit one intent."""
...
def _venue(now_ms: int, reference_price: float) -> VenueQualityV1:
    return VenueQualityV1(
        source="synthetic-current-release-gate",
...
@pytest.mark.asyncio
async def test_current_reject_all_path_is_deterministic_and_has_no_effect(tmp_path) -> None:
    ...
    route = CandidateRouterPolicy(source="current-release-gate-router").build(intent, TextAggregate())
    ...
    assert decision.exit_plan.model_dump(mode="json") == intent.exit_plan.model_dump(mode="json")
