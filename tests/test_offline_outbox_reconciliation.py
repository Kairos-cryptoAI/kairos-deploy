...
class OfflineOutboxWrapperTests(unittest.TestCase):
    def test_wrapper_is_explicit_profile_run_only_and_never_authorizes_a_row(self) -> None:
        ...
        self.assertIn("$ids = @(& docker ps --quiet", wrapper)
        self.assertIn("{{json .Config.Labels}}", wrapper)
        self.assertIn("$labels.'com.docker.compose.service'", wrapper)
        self.assertNotIn("{{.Label", wrapper)
        self.assertIn("[string]$NetworkName", wrapper)
        self.assertIn("Assert-IsolatedNetwork -NetworkName $DataNetwork", wrapper)
        self.assertNotIn("[string]$Network,", wrapper)
        self.assertIn("--profile $inspectProfile --profile $applyProfile", wrapper)
        self.assertIn("Validate both explicitly profiled services", wrapper)
        self.assertIn("function Get-SafeInspectionFailure", wrapper)
        self.assertIn('"classification,error_type,kind,schema_version,state"', wrapper)
        self.assertIn('"OFFLINE_EXACT_ROW_ONLY"', wrapper)
        self.assertIn('"STARTUP_REJECTED"', wrapper)
        self.assertIn("Read-only exact-row inspection rejected:", wrapper)
