"""Validate the fail-closed signed bounded offline outbox drain profile."""
...
POLICY_PATH = ROOT / "tests" / "offline_outbox_drain" / "policy.py"


def _load_policy() -> ModuleType:
    specification = importlib.util.spec_from_file_location("kairos_offline_outbox_drain_policy", POLICY_PATH)
    if specification is None or specification.loader is None:
        raise RuntimeError(f"could not load offline outbox drain policy from {POLICY_PATH}")
...
def _github_request(url: str, *, token: str | None) -> bytes:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "kairos-offline-outbox-drain-validator/1.0",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--source-lock", type=Path, default=Path("outbox-drain.sources.lock.json"))
    parser.add_argument("--packaged-lock", type=Path, default=Path("tests/offline_outbox_drain/source-lock.json"))
    parser.add_argument("--compose-json", type=Path)
    parser.add_argument("--normal-up-compose-json", type=Path)
    parser.add_argument("--dockerfile", type=Path, default=Path("tests/offline_outbox_drain/Dockerfile"))
    parser.add_argument("--dockerignore", type=Path, default=Path("tests/offline_outbox_drain/.dockerignore"))
    parser.add_argument("--trusted-signer", type=Path, default=Path("tests/offline_outbox_drain/trusted-signer.asc"))
    ...
    try:
        ...
        if packaged_lock != lock:
            errors.append("packaged offline outbox drain source lock differs from canonical lock")
        ...
        print(f"offline outbox drain validation could not start: {exc}", file=sys.stderr)
        return 2
    if errors:
        for error in errors:
            print(f"ERROR: {error}", file=sys.stderr)
        return 1
    print("Kairos offline signed-prefix outbox drain validation passed")
    return 0
